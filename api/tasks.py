...
def main(tweet_id):
    ...
    try:
        url, tweet_fields = create_url(tweet_id = tweet_id)
        ...
        while(json_response['meta']['next_token'] and status_code == 200):
            ...
            try:
                ...
                url, tweet_fields = create_url(tweet_id = tweet_id,pagination_token= json_response['meta']['next_token'])
                json_response,status_code = connect_to_endpoint(url, tweet_fields)
                result_array.extend(json_response['data'])
                # Response : {
    ...
    if len(result_array) > 0:
        ...
        file_path = os.path.join(os.getcwd(), 'media' ,  f"{tweet_id}_likes.csv")
        df.to_csv(file_path, index=False)
        # if not TweetLikes.objects.filter(tweet_id = tweet_id).exists():
        with default_storage.open(file_path, 'rb') as file:
            file_field = File(file)
            tweets = TweetLikes()
            tweets.tweet_id = tweet_id
            tweets.file.save(f"{tweet_id}_likes.csv", file_field)
            tweets.save()
